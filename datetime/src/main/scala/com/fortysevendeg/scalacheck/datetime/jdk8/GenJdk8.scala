...
object ArbitraryJdk8 extends GenJdk8 {
  implicit def arbJdk8ZonedDateTime(
      implicit granularity: Granularity[ZonedDateTime]): Arbitrary[ZonedDateTime] =
    Arbitrary(genZonedDateTime)

  implicit def arbJdk8LocalDateTime(
      implicit granularity: Granularity[LocalDateTime]): Arbitrary[LocalDateTime] =
    Arbitrary(genLocalDateTime)

  implicit def arbJdk8LocalDate(
      implicit granularity: Granularity[LocalDate]): Arbitrary[LocalDate] = Arbitrary(genLocalDate)

  implicit def arbJdk8LocalTime(
      implicit granularity: Granularity[LocalTime]): Arbitrary[LocalTime] = Arbitrary(genLocalTime)
